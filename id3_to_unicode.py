...
def unicode2bytestring( string ) :
    ...
    try :
        ...
        pass    # str fails chr(ord()) conversion
    return string
...
#   album = album.replace( '[+digital booklet]', '2010' )
    ...
    if overwrite_tags or not len(title) :
        title = str( tail, 'utf-8' )[:-4]   # strip trailing ".mp3"
    ...
    if overwrite_tags or not len(album) :
        album = str( tail, 'utf-8' )

    head, tail = os.path.split( head )
    if overwrite_tags or not len(artist) :
        artist = str( tail, 'utf-8' )
...
def collect_stats( file_name ) :
    global stats
    # if not eyed3.mp3.isMp3File( file_name ) :
    #     print(str( file_name, "utf-8" ), ': not an MP3 file')
    #     return
    ...
    try:
        ...
        if not audiofile.type == eyed3.core.AUDIO_MP3 :
            print(str( file_name, "utf-8" ), ': iD3 tag is missing')
            return

        tag = audiofile.tag
    except Exception as e :
        print(str( file_name, "utf-8" ), ':', str(e))
        return
...
def select_encoding( stats ) :
    ...
    # print(str(root,'utf-8'), ": several encodings are possible:")
    # for i in range(len(stats)) :
    #     print("%d. %s (%5.2f%%)" % (i+1, stats[i][1], stats[i][0]))

    while True :
        # print('select encoding (1..%d):' % (len(stats)), end=' ')
...
try:
    for root,dirs,files in os.walk( path ) :
        ...
        if len(stats) :
            ...
            print(str(root,'utf-8'), ': nothing to convert or already in unicode/ascii', end=' ')
        print()
        if not recursive :
            break
